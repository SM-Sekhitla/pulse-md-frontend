...
import { User as UserIcon, FileText, Pill, Plus, Trash2, Stethoscope, ShieldCheck } from "lucide-react";
import { AppShell } from "@/components/app-shell";
import {
  createPrescription, currentTenant, currentUser, patientAppointments, store,
  type PrescriptionItem,
} from "@/lib/store";

export const Route = createFileRoute("/prescriptions/new")({ component: NewPrescription });
...
function NewPrescription() {
  ...
  const medications = useMemo(
    () => s.inventory
      .filter(i => i.category === "Prescription medication" || i.category === "OTC medication")
  ...
  const addItem = () => setItems(prev => [...prev, { drug: "", dose: "", frequency: "", duration: "", notes: "" }]);
  ...
  const submit = (e: React.FormEvent) => {
    ...
    const cleanItems = items.filter(i => i.drug.trim()).map(i => ({
      drug: i.drug.trim(), dose: i.dose.trim(), frequency: i.frequency.trim(),
      duration: i.duration.trim(), notes: i.notes?.trim() || undefined,
    }));
    if (cleanItems.length === 0) { setError("Add at least one medication."); return; }
    ...
    <AppShell title="New prescription">
      <form onSubmit={submit} className="mx-auto max-w-[820px] space-y-5">
        <div className="pulse-card p-6">
          <h2 className="text-[16px] font-semibold text-navy">Issue a prescription</h2>
          <p className="mt-1 text-[13px] text-muted-foreground">Choose a patient, optionally link to an appointment, and add medications.</p>
          ...
          <div className="mt-5 space-y-5">
            <Section icon={<UserIcon className="h-4 w-4" />} title="Patient">
              ...
              {selectedPatient && (
                <div className="mt-2 rounded-md border border-border bg-surface px-3 py-1.5 text-[12px] text-muted-foreground">
                  ...
                  {selectedPatient.allergies.length > 0 && <span className="ml-2 text-[#991B1B]">⚠ Allergies: {selectedPatient.allergies.join(", ")}</span>}
                </div>
              )}
            </Section>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <Section title="ICD-10">
                <input value={icd10} onChange={(e) => setIcd10(e.target.value)} placeholder="e.g. J02.9"
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                ...
                <button type="button" onClick={addItem} className="inline-flex items-center gap-1 rounded-md border border-border bg-white px-2.5 py-1 text-[12px] font-medium text-navy hover:bg-surface">
                ...
                {items.map((it, i) => (
                  <div key={i} className="rounded-md border border-border bg-white p-3">
                    <div className="grid grid-cols-12 gap-2">
                      <select value={it.drug} onChange={(e) => updateItem(i, { drug: e.target.value })}
                        ...
                        <optgroup label="Prescription medication">
                          {medications.filter(m => m.category === "Prescription medication").map(m => (
                            <option key={m.id} value={m.name}>{m.name} {m.stock <= 0 ? "(out of stock)" : `· stock ${m.stock}`}</option>
                          ))}
                        </optgroup>
                        <optgroup label="OTC medication">
                          {medications.filter(m => m.category === "OTC medication").map(m => (
                            <option key={m.id} value={m.name}>{m.name} {m.stock <= 0 ? "(out of stock)" : `· stock ${m.stock}`}</option>
                          ))}
                        </optgroup>
                      </select>
                      <input value={it.dose} onChange={(e) => updateItem(i, { dose: e.target.value })} placeholder="Dose (1 tab)"
                        className="col-span-2 rounded-md border border-border bg-white px-2.5 py-1.5 text-[13px] outline-none focus:border-blue" />
                      <input value={it.frequency} onChange={(e) => updateItem(i, { frequency: e.target.value })} placeholder="Frequency (TDS)"
                        className="col-span-2 rounded-md border border-border bg-white px-2.5 py-1.5 text-[13px] outline-none focus:border-blue" />
                      <input value={it.duration} onChange={(e) => updateItem(i, { duration: e.target.value })} placeholder="Duration (5 days)"
                        className="col-span-2 rounded-md border border-border bg-white px-2.5 py-1.5 text-[13px] outline-none focus:border-blue" />
                    ...
                    <input value={it.notes || ""} onChange={(e) => updateItem(i, { notes: e.target.value })} placeholder="Notes (optional, e.g. take with food)"
            ...
            <div className="rounded-md border border-border bg-surface px-3 py-2 text-[12px] text-muted-foreground">
              ...
              On issue, a unique 12-character security code is generated and embedded in a QR on the printed prescription. Pharmacies scan it to verify authenticity.
            </div>

            {error && <div className="rounded-md bg-[#FEE2E2] px-3 py-2 text-[12.5px] text-[#991B1B]">{error}</div>}

            <div className="flex justify-end gap-2 pt-1">
              <button type="button" onClick={() => navigate({ to: "/prescriptions" })} className="rounded-md border border-border bg-white px-4 py-2 text-[13px] font-medium text-navy hover:bg-surface">Cancel</button>
