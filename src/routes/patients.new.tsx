      ...
      >
        <Section title="Personal details">
          <Grid>
            ...
            <Field label="Gender">
              <Sel options={["Female", "Male", "Other"]} />
            </Field>
            <Field label="ID number">
              <Inp placeholder="13 digits" />
            </Field>
          </Grid>
        </Section>
        ...
        <Section title="POPIA consent">
          <label className="flex items-start gap-2 text-[13px] text-navy">
            ...
            <span>
              I confirm that the patient has provided informed consent for the
              collection, processing, and storage of personal information in
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            type="submit"
            className="rounded-md bg-blue px-5 py-2 text-[13px] font-medium text-white hover:opacity-90"
          >
            Register patient
          </button>
        </div>
...
function Sel({ options }: { options: string[] }) {
  return (
    <select className="block w-full rounded-md border border-border bg-white px-3 py-2 text-[13px] outline-none focus:border-blue">
      {options.map((o) => (
