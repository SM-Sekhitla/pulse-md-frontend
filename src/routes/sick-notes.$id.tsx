...
function SickNoteDetail() {
  ...
  useEffect(() => {
    ...
    const loadNote = async () => {
      setLoading(true);
      const result = await sicknote.getSickNote(id);
      if (alive) {
        setNote(result);
        setLoading(false);
      }
  ...
  if (loading) {
    return (
      <AppShell title="Sick note">
        <div className="p-6">Loading sick note...</div>
      </AppShell>
    );
  }
    ...
    return (
      <AppShell title="Sick note">
        <div className="pulse-card p-8 text-center text-muted-foreground">
          Sick note not found.
          <div className="mt-4">
            <Link to="/sick-notes" className="text-blue hover:underline">
              Back to sick notes
            </Link>
          </div>
        </div>
  ...
  const days =
    differenceInCalendarDays(new Date(note.toDate), new Date(note.fromDate)) +
    1;
    ...
    <AppShell title="Sick note">
      ...
      <div className="mx-auto max-w-[820px]">
        <div className="rounded-lg border border-border bg-white p-10 shadow-sm print:border-0 print:shadow-none">
          ...
          <div className="mt-5 text-[13px] leading-relaxed text-navy">
            <p>
              ...
              <span className="font-semibold">{note.patientName}</span> was
              examined on{" "}
              <span className="font-semibold">
                {format(new Date(note.issuedAt), "d MMMM yyyy")}
              </span>{" "}
              and is medically unfit for work or school from{" "}
              <span className="font-semibold">
                {format(new Date(note.fromDate), "d MMMM yyyy")}
              </span>{" "}
              to{" "}
              <span className="font-semibold">
                {format(new Date(note.toDate), "d MMMM yyyy")}
              </span>{" "}
              ({days} day{days > 1 ? "s" : ""}).
            </p>
            <div className="mt-4 rounded-md border border-border bg-surface px-4 py-3">
              <div>
                <span className="label-caps text-muted-foreground">Reason</span>
                <span className="ml-2 text-navy">{note.reason}</span>
              </div>
              {note.icd10 && (
                <div className="mt-1">
                  ...
                  <span className="ml-2 font-mono text-navy">{note.icd10}</span>
                </div>
              )}
              {note.recommendation && (
                <div className="mt-1">
                  ...
                  <span className="ml-2 text-navy">{note.recommendation}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="mt-8 flex items-end justify-between border-t border-border pt-6">
            ...
            <div className="text-center">
              <DocumentQR src={note.qrCodeDataUrl} size={120} />
        ...
        <div className="mt-3 rounded-md border border-border bg-surface px-3 py-2 text-[11.5px] text-muted-foreground print:hidden">
          ...
          QR is generated by the backend and contains a signed verification
          token linked to{" "}
          {note.appointmentId ? "the booked appointment" : "this note record"},
          the issue date and the validity period.
        </div>
      </div>
    </AppShell>
  );
