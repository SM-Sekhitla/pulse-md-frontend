...
export const Route = createFileRoute("/admin/practices/")({ component: AllPractices });
  ...
  return (
    <AdminShell title="All practices">
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search practice or email…"
        ...
        <div className="ml-auto text-[12.5px] text-muted-foreground">{rows.length} result{rows.length === 1 ? "" : "s"}</div>
      </div>
      ...
      <div className="pulse-card overflow-hidden">
        <table className="w-full text-[13px]">
          <thead className="bg-surface text-[11px] uppercase tracking-wider text-muted-foreground">
            <tr>
              <Th>Practice</Th><Th>GP</Th><Th>Email</Th><Th>Plan</Th><Th>Status</Th><Th>Registered</Th><Th>Approved</Th><Th className="text-right">Patients</Th>
            </tr>
          </thead>
          <tbody>
            {rows.map(t => {
              const owner = s.users.find(u => u.id === t.gpUserId);
              return (
                <tr key={t.id} className="border-t border-border hover:bg-surface cursor-pointer">
                  <Td><Link to="/admin/practices/$id" params={{ id: t.id }} className="font-semibold text-navy hover:text-blue">{t.name}</Link></Td>
                  <Td>{owner?.title} {owner?.firstName} {owner?.lastName}</Td>
                  <Td className="text-muted-foreground">{owner?.email}</Td>
                  <Td>{t.plan}</Td>
                  <Td><Badge variant={STATUS_VARIANT(t.status)}>{t.status.replace("_", " ")}</Badge></Td>
                  <Td className="text-muted-foreground">{format(parseISO(t.createdAt), "d MMM yyyy")}</Td>
                  <Td className="text-muted-foreground">{t.approvedAt ? format(parseISO(t.approvedAt), "d MMM yyyy") : "—"}</Td>
                  <Td className="text-right font-mono">{s.patients.filter(p => !p.tenantId || p.tenantId === t.id).length}</Td>
                </tr>
              );
            })}
            {rows.length === 0 && <tr><td colSpan={8} className="px-5 py-10 text-center text-muted-foreground">No practices match your filters.</td></tr>}
          </tbody>
        </table>
      </div>
    </AdminShell>
