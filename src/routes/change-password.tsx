      ...
      <div className="flex flex-1 items-center justify-center px-4 pb-16">
        <div className="w-full max-w-[460px] pulse-card p-8">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-md bg-blue/10 text-blue"><ShieldCheck className="h-5 w-5" /></div>
            <div>
              <h1 className="text-[18px] font-semibold text-navy">Set a new password</h1>
              {required && <p className="text-[12px] text-muted-foreground">Required before accessing your dashboard.</p>}
            </div>
          </div>
          <p className="mt-4 text-[13px] text-muted-foreground">
            Signed in as <span className="font-medium text-navy">{user.email}</span>. Choose a password you'll remember — your temporary password will no longer work.
          </p>

          <form onSubmit={submit} className="mt-6 space-y-4">
            <PwField label="New password" value={pw} onChange={setPw} show={show1} toggle={() => setShow1(v => !v)} />
            <PwField label="Confirm new password" value={pw2} onChange={setPw2} show={show2} toggle={() => setShow2(v => !v)} />
          ...
          {required && (
            <button onClick={() => { logout(); navigate({ to: "/login" }); }} className="mt-5 block w-full text-center text-[12.5px] text-muted-foreground hover:text-navy underline">
              Sign out
            </button>
          )}
        </div>
