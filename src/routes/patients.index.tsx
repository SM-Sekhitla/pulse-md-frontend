...
function PatientsList() {
  ...
  const filtered = useMemo(() => {
    return data.patients.filter((p) => {
      const matchQ = q ? `${p.firstName} ${p.lastName} ${p.idNumber} ${p.phone}`.toLowerCase().includes(q.toLowerCase()) : true;
    ...
    <AppShell title="Patients">
      <div className="mb-5 flex items-end justify-between">
        <div>
          ...
          <h2 className="text-[22px] font-semibold text-navy">{filtered.length} patients</h2>
        </div>
        <div className="flex gap-2">
          ...
          <Link to="/patients/new" className="inline-flex items-center gap-1.5 rounded-md bg-blue px-3.5 py-2 text-[13px] font-medium text-white hover:opacity-90">
      ...
      <div className="pulse-card">
        <div className="flex flex-wrap items-center gap-3 border-b border-border px-5 py-3">
          <div className="relative flex-1 min-w-[240px]">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by name, ID, or phone…"
  ...
  return (
    <tr className="border-b border-border last:border-0 transition-colors hover:bg-blue-tint">
      <td className="px-5 py-3">
        <Link to="/patients/$id" params={{ id: p.id }} className="flex items-center gap-3 group">
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-blue-tint text-[12px] font-semibold text-blue">
            {p.firstName[0]}{p.lastName[0]}
          </div>
          <div>
            <div className="font-medium text-navy group-hover:text-blue">{p.firstName} {p.lastName}</div>
            <div className="text-[11.5px] text-muted-foreground">{p.gender === "F" ? "Female" : "Male"}</div>
          </div>
        </Link>
      </td>
      <td className="px-5 py-3 font-mono text-[12px] text-muted-foreground">{p.idNumber}</td>
      <td className="px-5 py-3 text-navy">{age}</td>
      <td className="px-5 py-3"><Badge variant="indigo">{p.medicalAid}</Badge></td>
      <td className="px-5 py-3 font-mono text-[12px] text-navy">{p.phone}</td>
      <td className="px-5 py-3 text-muted-foreground">{format(parseISO(p.lastVisit), "d MMM yyyy")}</td>
      <td className="px-5 py-3">
