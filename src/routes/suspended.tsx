    ...
    <div className="flex min-h-screen flex-col items-center bg-surface">
      ...
      <div className="flex flex-1 items-center justify-center px-4 pb-16">
        <div className="w-full max-w-[520px] pulse-card p-10 text-center">
          <div className="mx-auto mb-4 inline-flex h-12 w-12 items-center justify-center rounded-full bg-[#FEE2E2] text-[#991B1B]">!</div>
          <h1 className="text-[24px] font-semibold text-navy">Your account has been suspended</h1>
          <p className="mt-3 text-[14px] text-muted-foreground">
            Access to <span className="font-semibold text-navy">{tenant?.name || user?.practiceName}</span> is currently restricted.
          </p>
          {tenant?.suspensionReason && (
            <div className="mt-4 rounded-md border border-border bg-surface px-4 py-3 text-left text-[12.5px]">
              ...
              <div className="text-muted-foreground">{tenant.suspensionReason}</div>
            </div>
          )}
